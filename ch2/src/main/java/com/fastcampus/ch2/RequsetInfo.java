...
public class RequsetInfo {
	...
	public static void main(HttpServletRequest request) {
		...
		System.out.println("request.getContentLength() :" +request.getContentLength()); //요청 내용의 길이 알수없을때는 -1
		System.out.println("request.getContentType() :" +request.getContentType()); //요청 내용의 타입 알수 없을때는 null
		
		System.out.println("request. getMethod() :" +request. getMethod()); //요청방법
		System.out.println("request.getProtocol() :" +request.getProtocol()); //프로토콜 종류와 버전 http/1.1
		...
		System.out.println("request.getRequestURI() :" +request.getRequestURI()); //요청 url
		...
		System.out.println("request.getRemoteHost() :" +request.getRemoteHost()); //원격 호스트 또는 ip 주소
